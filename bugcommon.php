...
function checkopt ($dbh, $id, $opt) {
	if ($id == "" or $opt == "") return false;
	$sql = "select concat(opts,',',opts2) from metaman.d20_person where empnbr=$id";
	$sth = $dbh->prepare($sql);
	$sth->execute();
	$opts = $sth->fetchArray();
	if (substr_count($opts[0][0],$opt) == 1) return true;
	return false;
}

# return a standard <select> for a lookup table
function retselect ($dbh, $name, $tab, $def) {
...
function retselectfull ($dbh, $name, $tab, $def) {
	$out = "<select name='$name' id='$name'><option value=' '>--Select one--\n";
	#if ($def == 0)
	#	$out .= "<option value='0' selected>None\n";
	#else
	...
	$stmt = $dbh->query($sql);
    while ($row = $stmt->fetchAll(SQLITE3_NUM)) {
		list($cd,$descr,$active) = array($row[0],$row[1],$row[2]);
		if ($id == $def) $chk=" selected"; else $chk="";
...
function get_loc ($dbh, $id) {
	$sql = "select type_desc from metaman.d20_loc where type_id=".intval($id);
	$sth = $dbh->prepare($sql);
	$sth->execute();
	$loc = $sth->fetchArray();
	return $loc[0][0];
